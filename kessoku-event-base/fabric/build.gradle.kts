...
dependencies {
    common(project(":kessoku-event-base:common", configuration = "namedElements")) { isTransitive = false }
    shadowCommon(project(path = ":kessoku-event-base:common", configuration = "transformProductionFabric"))
}
...
val moveFiles = tasks.register<Copy>("moveFiles") {
    ...
    filteredFiles.files.forEach {
        from(it)
        into(Path.of(rootDir.path, "artifacts", "fabric"))
    }
}